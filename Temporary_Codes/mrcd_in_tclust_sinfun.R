...
.InitClusters <- function (X, iter, pa)
{
  ...
  for (k in seq_along(lista_matrix)) {
    ...
    mrcd <- CovMrcd(lista_matrix[[k]], alpha=1, h=NULL,
                     rho=NULL, maxcond=50, target=0,
                     maxcsteps=200, trace=as.integer(FALSE))
    iter$sigma[,,k] <- mrcd$cov
  }
  return (iter)
}
...
.mrcd_in_tclust_sinfun <- function (x, k, alpha, nstart = 50, iter.max = 20, equal.weights = FALSE, zero.tol = 1e-16)
{
  if (is.data.frame(x))
    x <- data.matrix(x)
  else if (!is.matrix (x))
...
#RICHIAMO IL .mrcd_in_tclust_sinfun()

tclust <- .mrcd_in_tclust_sinfun(x, k = 3, alpha = 0.05, nstart = 50, iter.max = 20, equal.weights = FALSE, zero.tol = 1e-16)
