...
.InitClusters <- function (X, iter, pa)
{
  ...
  for (k in seq_along(lista_matrix)) {
    mrcd <- .detmrcd(lista_matrix[[k]], alpha=1, h=NULL,
                     rho=NULL, maxcond=1000, target=0,
                     maxcsteps=200, trace=as.integer(FALSE))
    iter$center[k,] <- t(as.vector(mrcd$initmean))
    iter$sigma[,,k] <- mrcd$initcovariance
  }
  return (iter)
}
...
.mrcd_in_tclust_new <- function (x, k, alpha, nstart = 50, iter.max = 20, equal.weights = FALSE, zero.tol = 1e-16)
{
  if (is.data.frame(x))
    x <- data.matrix(x)
  else if (!is.matrix (x))
...
#RICHIAMO IL .mrcd_in_tclust_new()

tclust <- .mrcd_in_tclust_new(x, k = 3, alpha = 0.05, nstart = 50, iter.max = 20, equal.weights = FALSE, zero.tol = 1e-16)
