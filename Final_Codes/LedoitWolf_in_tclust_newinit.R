...
.InitClusters <- function (X, iter, pa)
{
  ...
  tclust_init <- tclust(X_init, k = pa$K, alpha = pa$alpha) 
...
.LedoitWolf_in_tclust_newinit <- function (x, k, alpha, nstart = 50, iter.max = 20, equal.weights = FALSE, zero.tol = 1e-16)
{
  if (is.data.frame(x))
    x <- data.matrix(x)
  else if (!is.matrix (x))
