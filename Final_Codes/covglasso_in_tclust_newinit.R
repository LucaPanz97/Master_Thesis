...
{
  p_init <- dim(X)[2]
  randomp_init <- sample(1:p_init, 10)
  X_init <- X[,randomp_init]
  tclust_init <- tclust(X_init, k = pa$K, alpha = pa$alpha, restr.fact = 50) 
  iter$assig <- tclust_init$cluster
...
.covglasso_in_tclust_newinit <- function (x, k, alpha, nstart = 50, iter.max = 20, equal.weights = FALSE, zero.tol = 1e-16, lambda, P)
{
  if (is.data.frame(x))
    x <- data.matrix(x)
  else if (!is.matrix (x))
...
#RICHIAMO IL .covglasso_in_tclust_newinit()

tclust <- .covglasso_in_tclust_newinit(x, k = 3, alpha = 0.05, nstart = 50, iter.max = 20, equal.weights = FALSE, zero.tol = 1e-16, lambda = 8, P = matrix(1, nrow = p, ncol = p))
